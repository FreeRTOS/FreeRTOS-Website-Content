...
本演示在 [ES449](http://www.softbaugh.com/ProductPage.cfm?strPartNo=ES449) 评估板
...
使用 [IAR Embedded Workbench for MSP430](https://www.iar.com/) 开发工具
和 SoftBaugh [FETP 并行
端口 JTAG 接口](http://www.softbaugh.com/ProductPage.cfm?strPartNo=FETP)。

在本移植和演示中，任务使用 MSP430 的 1 至 3 低功耗模式。

---

### 重要！使用 IAR MSP430 RTOS 移植的注意事项
...
2. [演示应用程序](#演示应用程序)
3. [配置和使用详情](#配置和使用详情)
4. [选择要使用的移植](#使用-msp430f449-以外的部件)
...
用于构建 MSP430 FreeRTOS 演示的 IAR 工作区名为 RTOSDemo.eww，位于
Demo/msp430_IAR 目录中。
...
* 并非所有的任务都会更新 LCD，因此没有可见的迹象表明它们是否正常运行。因此，我们创建了一项 'Check' 任务，
 ...
 则说明没有检测到错误。 如果切换频率提高到 500 毫秒，则表示 'Check' 任务
 发现了至少一个错误。
...
要生成演示应用程序：
1. 在 Embedded Workbench IDE 中打开工作区文件 FreeRTOS/Demo/MSP430_IAR/RTOSDemo.eww。
2. 选择需要的版本（调试或发布）。![](/media/2018/crossstudio.gif)
