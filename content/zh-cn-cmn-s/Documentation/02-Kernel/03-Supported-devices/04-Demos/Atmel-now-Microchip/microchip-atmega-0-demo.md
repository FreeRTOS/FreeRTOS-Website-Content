...
![ATmega4809 Curiosity Nano 开发板。](/media/2020/Screen-Shot-2020-08-21-at-3.24.28-PM.png)

  

有多个适用于 Microchip AVR 微控制器的 FreeRTOS 移植，本页描述了 ATmega-0 系列移植。 
Atmel AVR-GCC、XC8 和 AVR 的 [IAR 嵌入式工作区](https://www.iar.com/iar-embedded-workbench/#!?architecture=AVR) (IAR EWAVR) 等编译器均有可用移植。


### 支持的设备


本页描述的演示和示例均为 ATmega4809 配置，其具有 48 KB 闪存和 6 KB SRAM。虽然 
FreeRTOS 演示的大小取决于所使用的编译器，但在[完Full 演示](#full-演示) 配置中，它需要大约
15 KB 或更少的程序内存。除了空闲任务之外，运行 10 个实时任务需要约 2 KB SRAM。 
因此，剩余 SRAM 足够用于通信缓冲、高级算法和应用程序的其他 SRAM 密集型部件 
。


有关 ATmega-0 微控制器的更多信息，请访问
[ATmega-0 页面](https://www.microchip.com/design-centers/8-bit/avr-mcus/device-selection/atmega4809)。
...
* [Microchip MPLAB X](https://www.microchip.com/mplab/mplab-x-ide)
 （5.40，含设备包 ATmega_DFP 2.2.108）


用于 XC8 移植
* [Atmel Studio 7](https://www.microchip.com/mplab/avr-support/atmel-studio-7) 
 （7.0.2397，含设备包 ATmega_DFP 1.4.351） 
...
适用于 ATmega-0 微控制器的 FreeRTOS 演示应用程序（Blinky 演示、Minimal 演示、Full 演示）均面向 
[ATmega4809 Curiosity Nano 评估套件](https://www.microchip.com/DevelopmentTools/ProductDetails/PartNO/DM320115)。此等演示应用程序可以使用 MPLAB X (XC8)、Atmel Studio 
(AVR-GCC) ，或 IAR EWAVR 构建。


Atmel START 提供了更高级的应用程序示例。该示例面向以下三类板的组合： 
[Curiosity
 Nano Base for Click 板](https://www.microchip.com/developmenttools/ProductDetails/AC164162)、[ATmega4809 Curiosity Nano 评估套件](https://www.microchip.com/DevelopmentTools/ProductDetails/PartNO/DM320115)和 
[OLED1 Xplained Pro 板](https://www.microchip.com/developmenttools/ProductDetails/atoled1-xpro)。
...
FreeRTOS zip 文件下载内容包含所有 FreeRTOS 移植和演示应用程序的源代码——因此包含的文件比此项目所需文件多得多。请参阅[源代码组织](/Documentation/02-Kernel/06-Coding-guidelines/01-Source-code-organization)章节， 
获取目录结构的描述以及有关新建 FreeRTOS 项目的信息。


ATmega4809 演示应用程序的 MPLAB X 项目位于 `FreeRTOS/Demo/AVR_ATMega4809_MPLAB.X` 目录下。


ATmega4809 演示应用程序的 Atmel Studio 7 项目名为 `RTOSDemo.atsln`，位于 
`FreeRTOS/Demo/AVR_ ATMega4809_AtmelStudio` 目录下。


ATmega4809 演示应用程序的 IAR EWAVR 项目名为 `RTOSDemo.eww`，位于 
`FreeRTOS/Demo/AVR_ ATMega4809_IAR` 目录下。
...
* `The main_blinky()` 函数创建一个队列和两个任务，然后启动 RTOS 调度器。
* 队列发送任务通过 `main_blinky.c` 中的 `prvQueueSendTask()` 实现。此任务每 200 毫秒向队列发送一个固定值 
 (100)。
* 队列接收任务通过 `main_blinky.c` 中的 `prvQueueReceiveTask()` 实现。此任务在队列读取时进入阻塞状态， 
 并在每次从队列发送任务发出的消息被接收时，解除阻塞并切换 LED。队列发送任务 
 每 200 毫秒向队列发送一次消息，因此队列接收任务每 200 毫秒解除阻塞状态并切换一次 LED 
 。
...
* 整数数学测试通过 `integer.c` 文件中的 `vStartIntegerMathTasks()` 实现。 
 此测试创建一个任务函数，它重复执行 32 位计算并检查结果是否符合预期 
 。
* 寄存器测试通过 `regtest.c` 文件中的 `vStartRegTestTasks()` 实现。此测试创建 
 两个任务，它们在所有寄存器中写入不同的值并验证其内容是否在上下文切换期间被保留 
 。
* 轮询队列测试通过 `PollQ.c` 文件中的 `vStartPolledQueueTasks()` 实现。它 
 创建两个任务，它们通过单个队列进行通信。其中一个任务作为生产者，另一个作为消费者， 
 所有队列访问都不会被阻塞。
* 串行通信测试通过 `comtest.c` 文件中的 `vAltStartComTestTasks()` 实现。
 此测试创建两个任务，它们在中断驱动程序串口上运行。为确保所有传输内容都被接收， 
 应使用环回连接器。
...
如果 `mainSELECTED_APPLICATION` 设置为 2，则 `main()` 调用 `main_full()` 以创建一个综合 
演示，用于演示和测试许多 FreeRTOS 功能，包括[任务](/Documentation/02-Kernel/02-Kernel-features/01-Tasks-and-co-routines/01-Tasks-overview)、 
...
* 通过 `semtest.c` 文件中的 `vStartSemaphoreTasks()` 实现的信号量测试任务各创建 
 两个任务集，每个任务集包含两个任务。任务集内的任务共享一个变量，其访问由信号量保护。
* 通过 `TaskNotify.c` 文件中的 `vStartTaskNotifyTask()` 实现的直达任务通知任务 
 创建一个单独执行某些测试的任务，之后再循环操作，通过软件定时器和中断来接收通知 
 。
* 通过 `Regtest.c` 文件中的 `vStartRegTestTasks()` 实现的寄存器测试任务创建两个任务， 
 这些任务在所有寄存器中写入不同的值并验证寄存器的内容是否在上下文切换期间被保留
 。
* 通过 `RecMutex.c` 文件中的 `vStartRecursiveMutexTasks()` 实现的递归互斥锁任务
...
IAR EWAVR。从演示代码角度来看，三个演示项目都是相同的。此外， 
...
[https://github.com/FreeRTOS/FreeRTOS](https://github.com/FreeRTOS/FreeRTOS)）。FreeRTOS 下载包含用于多个处理器移植和演示应用程序的源代码， 
但每个支持的处理器架构都需要少量特定于架构的 RTOS 代码。这 
是 RTOS 可移植层，位于 `FreeRTOS-Kernel/Source/Portable/<compiler>/<architecture>` 
子目录，其中 \<compiler\> and \<architecture\> 分别是用于创建移植的编译器和移植运行的架构 
...
位于 `.../Portable/GCC/AVR_Mega0` 文件夹的移植文件。
...
ATmega-0 移植将 '`BaseType_t`' 定义为 char（8 位）： 


* `#define portBASE_TYPE char`


注意：tick 定时器选项列表包含可用于 ATmega-0 系列中较大成员的选项。请 
参阅可用选项列表中的特定设备数据表。
...
`Source/Portable/MemMang/heap_1.c` 包含在 ATmega4809 演示应用程序项目中，以提供 
...
* 演示应用程序包括一个名为 `partest.c` 的文件，该文件是 MCU 的完整移植的驱动程序。 
 ...
 首先要让 LED 输出正常工作，因此我们选择了 LED 引脚。对于选定的硬件平台 (ATmega4809 Curiosity 
...
之所以选择 ATmega4809 Curiosity Nano 是因为其功能易于使用， 
但您也可以轻松使用其他任何硬件平台。tick 定时器的多个可用选项以及可用演示有助于 
在 ATmega-0 系列中的任何设备上开发 FreeRTOS 应用程序。
...
然后重新编译项目即可。使用的任务数量以及应用程序的复杂性 
决定了程序和数据内存的大小，从而表明可以使用哪个设备。
...
适用于所有 ATmega-0 设备。您也可以使用 `configUSE_TIMER_INSTANCE` 来选择其他定时器，如下所示（请参阅 
`FreeRTOS_Config.h` 文件了解可用选项）：


* `#define configUSE_TIMER_INSTANCE 0`：使用 TCB0 作为 tick 定时器
* `#define configUSE_TIMER_INSTANCE 1`：使用 TCB1 作为 tick 定时器
* `#define configUSE_TIMER_INSTANCE 2` ：使用 TCB2 作为 tick 定时器
* `#define configUSE_TIMER_INSTANCE 3`：使用 TCB3 作为 tick 定时器
* `#define configUSE_TIMER_INSTANCE 4`：使用 RTC 作为 tick 定时器


#### 时钟配置


ATmega-0 移植使用片上振荡器作为主时钟。考虑到其内部架构，内部时钟只能 
使用特定的预定义值。为方便您使用 ATmega-0 平台，我们还提供了一个 
...
该驱动程序使用了 USART3 实例（文件 `serial.c`）。必要时，任何其他可用 USART 实例 (USART0-3) 
