---
title: "Infineon XMC4500 ARM Cortex-M4F 演示使用 GCC 和 Atollic 开发工具"
...
本页记录的演示应用程序使用 FreeRTOSGCC ARM Cortex-M4F 移植，
面向 Infineon XMC4000 Hexagon 开发套件 CPU 开发板，
该板装有 [XMC4500 微控制器](http://www.infineon.com/xmc)。

要使用该移植，必须打开硬件浮点单元（FPU 或 VFP），
并且必须设置 GCC 编译时间选项以生成硬件浮点指令。
FreeRTOS 移植层本身会在 RTOS 调度程序启动之前立即打开 FPU，
并且本页描述的预配置演示应用程序具有所有
必需的编译器选项。
...
### *重要提示！关于使用 FreeRTOS GCC XMC4000 演示项目的说明*
...
此演示应用程序的 Eclipse 项目文件位于
FreeRTOS/Demo/CORTEX_M4F_Infineon_XMC4500_GCC_Atollic
目录中。本页的
[准备 Eclipse 项目](#构建并执行演示应用程序)部分
包含有关设置演示项目目录，
并将演示项目导入 Atollic IDE 的重要信息。

---

### Infineon XMC4500 演示应用程序

单个 Ecplise 项目提供了两种配置。可配置
为运行简单的 "blinky" 风格的项目或者更全面的测试和演示应用程序。
...
* **队列发送任务：**
 ...
 它会反复阻塞 200 毫秒，然后将数值 100 发送到
 在 main_blinky() 中创建的队列。一旦值发送成功，任务就会再次循环，
 再次被阻塞 200 毫秒。
* **队列接收任务：**
...
mainCREATE_SIMPLE_BLINKY_DEMO_ONLY 设置为 0 会导致 main() 调用
main_full()。main_full() 会创建更全面的测试和演示应用程序，演示：
...
FreeRTOS Atollic ARM CORTEX-M4F 项目使用托管 make 项目。这
反过来又意味着：
...
此演示采用上方的选项 1。因此，目录 FreeRTOS/Demo/CORTEX_M4F_Infineon_XMC4500_GCC_Atollic
包含一个名为 CreateProjectDirectoryStructure.bat 的批处理文件，
该文件将所需的所有 FreeRTOS 源文件和一些标准的演示应用程序文件
复制到演示项目目录下的子目录中。

**CreateProjectDirectoryStructure.bat 必须在将 Atollic
...
1. 在 Atollic 的 "File" 菜单中选择 "Import"。系统将显示
...
**首次点击 "Import" 时显示的对话框**
2. 在下一个对话框中，选择 FreeRTOS/Demo/CORTEX_M4F_Infineon_XMC4500_GCC_Atollic
...
![选择将 ARM Cortex-M4 RTOS演示导入 Eclipse](/media/2018/selecting-the-FreeRTOS-STM32-project-to-import-into-Eclipse.jpg)

**确保勾选 RTOS 演示项目（而非 FreeRTOS-Simple-Demo
...
此演示的特定配置项目位于 FreeRTOS/Demo/CORTEX_M4F_Infineon_XMC4500_GCC_Atollic/FreeRTOSConfig.h 中。
