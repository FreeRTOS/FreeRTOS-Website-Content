...
所有四个详细级别。完成此操作后，可通过定义 LIBRARY_LOG_LEVEL 单独设置详细级别，
如下所述。
...
需要在 `logging_stack.h` 中包含 `core_mqtt_config.h`并定义日志记录宏。

要获取日志记录，必须定义 SkdLog() 以调用线程安全平台特定的打印函数。例如，
打印函数可能会将字符输出到串行端口或 TCP 套接字。由于日志记录宏
接受可变数量的参数，并且使用方式与 printf() 相同，因此平台特定的
打印函数必须具有与 printf() 相同的原型（参数列表）。例如，如果应用程序
具有写入串行端口的线程安全版 printf()，则可以将 SdkLog 定义为：
...
要设置详细级别，请在用于定义 SdkLog() 的同一配置文件中
...
和 LOG_METADATA_ARGS 宏。LOG_METADATA_FORMAT 宏应被定义为指定
元数据格式字符串，而 LOG_METADATA_ARGS 宏应被定义为
传递格式字符串的元数据实参。这两个宏会作为前缀添加到日志消息中，然后传递给
针对 SdkLog 宏定义的平台特定的打印函数。
...
请注意，在下方示例链接和模板代码中，日志记录宏定义周围有 `#ifndef` 预处理指令
，旨在避免在包含多个配置文件的转换单元中
