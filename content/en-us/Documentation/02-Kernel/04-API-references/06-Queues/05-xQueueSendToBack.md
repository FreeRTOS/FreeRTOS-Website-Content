...
+ *xTicksToWait* 
  ...
  in tick periods so the constant portTICK\_PERIOD\_MS should be used to convert to real time if this 
  is required. If [INCLUDE\_vTaskSuspend](/Documentation/02-Kernel/03-Supported-devices/02-Customization) is set to '1' then specifying the block time as 
  portMAX\_DELAY will cause the task to block indefinitely (without a timeout).
