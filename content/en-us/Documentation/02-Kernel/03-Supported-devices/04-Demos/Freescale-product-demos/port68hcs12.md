---
title: "Motorola/Freescale HCS12 RTOS Port banked memory model"
created: 2018-09-20
categories:
  - kernel
relatedLinks:
...
memory model on an MC9S12DP256B processor. See the [MC9S12C32 RTOS port](porthcs12) page
for an example that uses the small memory model.

The port was developed on a [M68KIT912DP256](http://www.freescale.com/webapp/sps/site/prod_summary.jsp?code=M68KIT912DP256&parentCode=MC9S12A512&nodeId=0162468636K100)
development kit from Freescale ([instructions](/Documentation/02-Kernel/03-Supported-devices/04-Demos/04-Modifying-demos) are provided should you wish to use an alternative development board). It uses the
[CodeWarrior HC12 Development Tools](http://www.codewarrior.com/MW/Develop/Embedded/HC12/Default.htm)
 and a P&E Multilink USB BDM interface - both
...
The HCS12 CodeWarrior banked memory model demo application project file is located in the FreeRTOS/Demo/HCS12\_CodeWarrior\_banked directory
...
* LEDs D0, D1 and D2 are under control of the 'flash' tasks. Each will flash at a constant frequency, with LED D0
 being the fastest and LED D2 being the slowest.
* When the loopback connector is in place every character transmitted by the ComTest Tx task is received by the ComTest
 Rx task. Each transmitted character causes LED D3 to toggle, each correctly received character causes LED D4 to
 toggle. LED D4 will only toggle if the received character matches the character the ComTest Rx task expected to receive.
* Not all the tasks update an LED so have no visible indication that they are operating correctly.
 ...
 LED D7 is under control of the 'Check' task. Every three
 seconds the 'Check' task examines all the tasks in the system to ensure they are executing without error. It
 then toggles LED D7. If LED D7 is toggling every three seconds then no errors have ever been detected.
 The toggle rate increasing to 500ms indicates that the 'Check' task has
 discovered at least one error.

#### Building the demo application

The RTOSDemo.mcp project file (located in the FreeRTOS/Demo/HCS12\_CodeWarrior\_banked directory) should be opened from within
the CodeWarrior IDE.
...
* BANKED\_MODEL must be defined in the file FreeRTOS/Demo/HCS12\_CodeWarrior\_banked/FreeRTOSConfig.h.
...
Configuration items specific to this port are contained in FreeRTOS/Demo/HCS12\_CodeWarrior\_banked/FreeRTOSConfig.h. The
...
Set the definition configUSE\_PREEMPTION within FreeRTOS/Demo/HCS12\_CodeWarrior\_banked/FreeRTOSConfig.h to 1 to use pre-emption or 0
to use co-operative.
...
FreeRTOS/Source/Portable/MemMang/heap\_2.c is included in the HCS12 demo application project to provide the
