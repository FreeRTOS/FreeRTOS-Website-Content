---
title: "FreeRTOS for Renesas RX231 (RXv2) Supporting GCC, IAR and Renesas compilers"
created: 2018-09-20
categories:
  - kernel
relatedLinks:
...
![Renesas RX231 RXv2 core](/media/2018/Renesas_Generic.jpg)
...
family of microcontrollers, which have an RXv2 core.
...
or a comprehensive demo, and target the RX231 RSK (Renesas Start Kit) evaluation
board.

---

#### *IMPORTANT! Notes on using the RX231 RTOS demo projects*
...
contains lots more files than are required by the RX231 projects.
...
/FreeRTOS/Demo/RX200\_RX231-RSK\_GCC\_e2studio\_IAR directory.
...
/FreeRTOS/Demo/RX200\_RX231-RSK\_GCC\_e2studio\_IAR directory, and the
project that uses the Renesas compiler is located in the
/FreeRTOS/Demo/RX200\_RX231-RSK\_Renesas\_e2studio directory. These are
the directories that must be selected when importing the
projects into the e2studio Eclipse workspace.

---

### Building and Running the Renesas RX231 RTOS Demo

The RTOS demo projects can be configured to build either
a simple blinky application, or a comprehensive test and demo application.
The constant
...
The demos use an LED built onto the RSK development board, so no hardware
set up is required.
...
![Importing the RX64M project into the e2studio Eclipse IDE](/media/2018/E2Studio_import.jpg)

**The dialogue box that appears when "Import" is first clicked**
3. In the next dialogue box, select /FreeRTOS/Demo/RX200\_RX231-RSK\_GCC\_e2studio\_IAR
 as the root directory if you are using the GCC compiler, or
 /FreeRTOS/Demo/RX200\_RX231-RSK\_Renesas\_e2studio if you are using
...
7. Ensure the E1 debug adapter (which comes with the RSK starter kit) is
 connected between the RX231 RSK board and the computer running e2studio.
...
* **The main\_full() Function:**
 ...
 test tasks, a pseudo randomiser task, and
...
#### RX231 RTOS port specific configuration

Configuration items specific to this demo are contained in /FreeRTOS/Demo/RX200\_RX231-RSK\_GCC\_e2studio\_IAR/src/FreeRTOSConfig.h
for the IAR and GCC projects, and /FreeRTOS/Demo/RX200\_RX231-RSK\_Renesas\_e2studio/src/FreeRTOSConfig.h for the Renesas compiler
project. The
...
* **configKERNEL\_INTERRUPT\_PRIORITY**
 ...
 the lowest interrupt priority, which is 1 for the RX231. See [the configuration pages](/Documentation/02-Kernel/03-Supported-devices/02-Customization/#configkernel_interrupt_priority-configmax_syscall_interrupt_priority-and-configmax_api_call_interrupt_priority) for more information.
...
included in main.c within each RX200 demo application.
...
Source/Portable/MemMang/heap\_4.c is included in the RX231 demo application project to provide the memory
