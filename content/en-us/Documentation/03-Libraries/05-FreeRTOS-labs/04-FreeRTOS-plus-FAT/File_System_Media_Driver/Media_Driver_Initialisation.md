---
title: "Creating a Media Driver: The Driver's Initialisation Function"
created: 2018-09-20
categories:
  - kernel
relatedLinks:
...
*Actions performed within a media driver's initialisation function (optional steps)*

The media driver's initialisation function must allocate an [FF\_Disk\_t](FF_Disk_t)
structure for use with the media.

The FF\_Disk\_t structure contains a pointer to an FF\_IOManager\_t structure.
The FF\_IOManager\_t structure is created by calling [FF\_CreateIOManager()](FF_CreateIOManager).

For convenience, the media driver can
optionally also [mount a partition](/Documentation/03-Libraries/05-FreeRTOS-labs/04-FreeRTOS-plus-FAT/Native-API/FF_Mount)
on the media, and [add the mounted partition to FreeRTOS-Plus-FAT's virtual file system](/Documentation/03-Libraries/05-FreeRTOS-labs/04-FreeRTOS-plus-FAT/Native-API/FF_FS_Add).
Performing these two optional steps within the initialisation function removes the need for
...
function used by the FreeRTOS-Plus-FAT RAM disk driver. The full version,
which includes additional error checking, can be found
in /FreeRTOS-Plus/Source/FreeRTOS-Plus-FAT/portable/common/ff\_ramdisk.c.
...
FF_CreationParameters_t xParameters;
    ...
    if( pxDisk != NULL )
    {
        ...
        pxDisk->pxIOManager = [FF\_CreateIOManger](FF_CreateIOManager)( &xParameters, &xError );
