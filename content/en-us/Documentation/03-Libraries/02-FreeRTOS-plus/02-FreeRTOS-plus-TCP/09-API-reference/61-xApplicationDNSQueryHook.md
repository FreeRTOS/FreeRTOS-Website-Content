...
**Note**: this hook needs to be defined only when `ipconfigUSE_NBNS` or `ipconfigUSE_LLMNR` 
is set to 1 in the FreeRTOSIPConfig.h file.
  
```c
BaseType_t xApplicationDNSQueryHook( const char * pcName );
```

xApplicationDNSQueryHook is an application defined hook (or callback) function that is called by the 
FreeRTOS-Plus-TCP stack to check whether the LLMNR or NBNS name received is the same as the one device 
is looking for.
...
in an application hook should not call FreeRTOS-Plus-TCP API's that are blocking. That could easily lead 
to a dead-lock.
