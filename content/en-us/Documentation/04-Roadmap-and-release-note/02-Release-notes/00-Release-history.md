---
title: "FreeRTOS Release History"
created: 2018-09-20
categories:
  - roadmap and release notes
description: Information on FreeRTOS release history
relatedLinks: 
...
### Changes between FreeRTOS V11.3.0 and FreeRTOS V11.3.1 released August 21, 2026
...
### Changes between FreeRTOS V11.2.0 and FreeRTOS V11.3.0 released March 30, 2026
...
+ Add FPU Safe Application IRQ Handler support to the ARM_AARCH64_SRE port.
...
+ Optimize certain getter APIs by removing unnecessary calls to
  taskENTER|EXIT_CRITICAL() when the data access is atomic. We thank
...
+ Add runtime parameter checks to functions in mpu\_wrappers\_v2.c file.
...
+ Add a new MPU wrapper that places additional restrictions on unprivileged
  ...
  3. Execute system calls on a separate privileged only stack: FreeRTOS
     ...
     the system call stack using new configSYSTEM\_CALL\_STACK\_SIZE config
     macro.
...
+ Add ulTaskGetRunTimeCounter and ulTaskGetRunTimePercent APIs. Contributed by @chrisnc.

+ Add APIs to get the application supplied buffers from statically created kernel objects. 
...
+ Add configTICK\_TYPE\_WIDTH\_IN\_BITS to configure TickType\_t data type. As a result,
...
+ Update heap\_4 and heap\_5 to add padding only if the resulting block is not
  already aligned.
...
+ Add assertions to ARMv8-M ports to detect when FreeRTOS APIs are called from
  interrupts with priority higher than the configMAX\_SYSCALL\_INTERRUPT\_PRIORITY.
  Contributed by @urutva.

+ Add xPortIsInsideInterrupt API to ARM\_CM0 ports.
...
+ Update ARMv7-M ports to ensure that kernel interrupts run at the lowest priority.
  configKERNEL\_INTERRUPT\_PRIORITY is now obsolete for ARMv7-M ports and brings
...
+ Add portMEMORY\_BARRIER to Microblaze port. Contributed by @bbain.

+ Add portPOINTER\_SIZE\_TYPE definition for ATmega port. Contributed by @jputcu.
...
+ ARMv7-M and ARMv8-M MPU ports: It was possible for an unprivileged task
  ...
  MPU\_xTaskCreate, MPU\_xTaskCreateStatic, MPU\_xTimerCreate,
  MPU\_xTimerCreateStatic, or MPU\_xTimerPendFunctionCall. MPU\_xTaskCreate
  and MPU\_xTaskCreateStatic have been updated to only allow creation of
  unprivileged tasks. MPU\_xTimerCreate, MPU\_xTimerCreateStatic and
  MPU\_xTimerPendFunctionCall APIs have been removed.
...
+ Heap improvements:
  ...
  - Add a check to heap\_2 to track if a memory block is allocated to
    the application or not. The MSB of the size field is used for this
    purpose. The same check already exists in heap\_4 and heap\_5. This
    check prevents double free errors.

  - Add a new flag configHEAP\_CLEAR\_MEMORY\_ON\_FREE to heap\_2, heap\_4
    and heap\_5. If the flag is set in FreeRTOSConfig.h then memory freed using
    vPortFree() is automatically cleared to zero.

  - Add a new API pvPortCalloc to heap\_2, heap\_4 and heap\_5 which has the same
    signature as the standard library calloc function.

  - Update the pointer types to portPOINTER\_SIZE\_TYPE. Contributed by
    @Octaviarius.
...
+ Add the ability to override send and receive completed callbacks for each
  ...
  configUSE\_SB\_COMPLETED\_CALLBACK in FreeRTOSConfig.h. When the option is set to 1,
  ...
  sbSEND\_COMPLETED() and sbRECEIVE\_COMPLETED() macros are invoked. To maintain
  backwards compatibility, configUSE\_SB\_COMPLETED\_CALLBACK defaults to 0. The
...
+ Add ARM Cortex-M7 r0p0/r0p1 Errata 837070 workaround to ARM CM4 MPU ports.
  The application writer needs to define configENABLE\_ERRATA\_837070\_WORKAROUND
  when using CM4 MPU ports on a Cortex-M7 r0p0/r0p1 core.

+ Add configSYSTICK\_CLOCK\_HZ to Cortex-M0 ports. This is needed to support
...
+ Introduce the configUSE\_MINI\_LIST\_ITEM configuration option. When this
  option is set to 1, ListItem\_t and MiniLitItem\_t remain separate types.
  However, when configUSE\_MINI\_LIST\_ITEM == 0, MiniLitItem\_t and ListItem\_t
  are both typedefs of the same struct xLIST\_ITEM. This addresses some issues
  observed when strict-aliasing and link time optimization are enabled.
  To maintain backwards compatibility, configUSE\_MINI\_LIST\_ITEM defaults to 1.
...
+ Changed uxAutoReload parameter in timer functions to xAutoReload.  The
  type is now BaseType\_t.  This matches the type of pdTRUE and pdFALSE.
  The new function xTimerGetAutoReload() provides the auto-reload state as
  a BaseType\_t.  The legacy function uxTimerGetAutoReload is retained with the
  original UBaseType\_t return value.
...
+ Fix compilation error in tasks.c when configSUPPORT\_DYNAMIC\_ALLOCATION
  is set to zero. Contributed by @rdpoor.

+ Fix prvWriteMessageToBuffer() function in stream\_buffer.c so that it correctly
  copies length on big endian platforms too.

+ Remove the need for  INCLUDE\_vTaskSuspend to be set to 1
  when configUSE\_TICKLESS\_IDLE is enabled. Contributed by @pramithkv.
...
+ Introduce a new config configALLOW\_UNPRIVILEGED\_CRITICAL\_SECTIONS which
...
+ Introduce configRUN\_TIME\_COUNTER\_TYPE which enables developers to define
  the type used to hold run time statistic counters. Defaults to uint32\_t
  for backward compatibility. #define configRUN\_TIME\_COUNTER\_TYPE to a type
  (for example, uint64\_t) in FreeRTOSConfig.h to override the default.
...
+ Introduce ulTaskGetIdleRunTimePercent() to complement the pre-existing
  ...
  configIDLE\_SHOULD\_YIELD is set to 0.
...
+ ARMv8-M secure-side port:  Tasks that call secure functions from the
  ...
  introduction of the secureconfigMAX\_SECURE\_CONTEXTS configuration constant,
  which sets the number of statically allocated secure contexts.
  secureconfigMAX\_SECURE\_CONTEXTS defaults to 8 if left undefined.
...
+ Minor refactor of timers.c that obsoletes the need for the
  tmrCOMMAND\_START\_DONT\_TRACE macro and removes the need for timers.c to
...
+ The xTaskCreate() and xTaskCreateStatic() functions accept a task priority
  ...
  (configMAX\_PRIORITIES - 1) should it be set to a value above that priority.
...
+ Introduce the configSTACK\_ALLOCATION\_FROM\_SEPARATE\_HEAP configuration
...
+ Task notifications:  Prior to FreeRTOS V10.4.0 each created task had a
  ...
  [RTOS Task Notifications](/Documentation/02-Kernel/04-API-references/05-Direct-to-task-notifications/00-RTOS-task-notifications)
...
+ Kernel ports that support memory protection units (MPUs): The ARMv7-M and
  ...
  See [Memory Protection Unit (MPU) Support](/Security/04-FreeRTOS-MPU-memory-protection-unit)
  for more information.
...
See https://www.FreeRTOS.org/Documentation/04-Roadmap-and-release-note/02-Release-notes/00-Release-history#changes-between-freertos-v1030-and-freertos-v1031-released-february-18-2020
...
See https://www.FreeRTOS.org/Documentation/04-Roadmap-and-release-note/02-Release-notes/04-FreeRTOS-V10.3.0
...
+ Correct alignment of stack top in RISC-V port when
  configISR\_STACK\_SIZE\_WORDS is defined to a non zero value, which causes
...
+ Updated the behaviour of the ARMv7-M MPU (Memory Protection Unit) ports to
  ...
  configENFORCE\_SYSTEM\_CALLS\_FROM\_KERNEL\_ONLY configuration constant.
...
+ Change type of uxPendedTicks from UBaseType\_t to TickType\_t to ensure it
...
+ Added CORTEX\_MPU\_STM32L4\_Discovery\_Keil\_STM32Cube demo.
...
+ Introduced the portMEMORY\_BARRIER macro to prevent instruction re-ordering
  when GCC link time optimisation is used.

+ Introduced the portDONT\_DISCARD macro to the ARMv8-M ports to try and
  prevent the secure side builds from removing symbols required by the
  non secure side build.

+ Introduced the portARCH\_NAME to provide additional data to select semi-
...
+ Added configINCLUDE\_PLATFORM\_H\_INSTEAD\_OF\_IODEFINE\_H to the Renesas
...
+ Added uxTaskGetStackHighWaterMark2() function to enable the return type to
  ...
  uxTaskGetStackHighWaterMark() returns a UBaseType\_t as always,
  uxTaskGetStackHighWaterMark2() returns configSTACK\_DEPTH\_TYPE to allow the
...
+ Fixed issues in memory protected ports related to different combinations
  ...
  definition of tskSTATIC\_AND\_DYNAMIC\_ALLOCATION\_POSSIBLE became more
...
+ Introduced portHAS\_STACK\_OVERFLOW\_CHECKING, which should be set to 1 for
...
+ Updated all object handles (TaskHandle\_t, QueueHandle\_t, etc.) to be
  ...
  debuggers).  Note this required the pvContainer member of a ListItem\_t
  struct to be renamed - set configENABLE\_BACKWARD\_COMPATIBILITY to 1 if
  this causes an issue.

+ Added configUSE\_POSIX\_ERRNO to enable per task POSIX style errno
...
+ The usStackHighWaterMark member of the TaskStatus\_t structure now has type
  configSTACK\_DEPTH\_TYPE in place of uint16\_t - that change should have been
  made when the configSTACK\_DEPTH\_TYPE type (which gets around the previous
...
+ Introduce configMESSAGE\_BUFFER\_LENGTH\_TYPE to allow the number of bytes
  used to hold the length of a message in the message buffer to be reduced.
  configMESSAGE\_BUFFER\_LENGTH\_TYPE default to size\_t, but if, for example,
  messages can never be more than 255 bytes it could be set to uint8\_t,
  saving 3 bytes each time a message is written into the message buffer
  (assuming sizeof( size\_t ) is 4).

+ Updated the StaticTimer\_t structure to ensure it matches the size of the
  Timer\_t structure when the size of TaskFunction\_t does not equal the size
  of void *.
...
+ Fix position of "#if defined( \_\_cplusplus )" in stream\_buffer.h.

+ Correct declarations of MPU\_xQueuePeek() and MPU\_xQueueSemaphoreTake() in
  mpu\_prototypes.h.

+ Correct formatting in vTaskList() helper function when it prints the state
  of the currently executing task.

+ Introduce #error if stream\_buffer.c is built without
  configUSE\_TASK\_NOTIFICATIONS set to 1.
...
+ Update FreeRTOS+TCP to V2.0.0
  ...
  - Introduce ipconfigSOCKET\_HAS\_USER\_WAKE\_CALLBACK option to enable a user
...
+ Stream Buffers - see https://www.FreeRTOS.org/Documentation/02-Kernel/02-Kernel-features/04-Stream-and-message-buffers/02-Stream-buffer-example
...
+ Move FreeRTOS+TCP into the main repository, along with the basic Win32
  TCP demo FreeRTOS\_Plus\_TCP\_Minimal\_Windows\_Simulator.
...
+ Added CORTEX\_MPU\_Static\_Simulator\_Keil\_GCC demo to test static allocation
  in the MPU port.
...
+ Update mpu\_wrappers.c to support static allocation.

+ The uxNumberOfItems member of List\_t is now volatile - solving an issue

  when the IAR compiler was used with maximum optimization.

+ Introduced configRECORD\_STACK\_HIGH\_ADDRESS.  When set to 1 the stack start
  address is saved into each task's TCB (assuming stack grows down).

+ Introduced configINCLUDE\_FREERTOS\_TASK\_C\_ADDITIONS\_H to allow user defined
  functionality, and user defined initialisation, to be added to FreeRTOS's
  tasks.c source file.  When configINCLUDE\_FREERTOS\_TASK\_C\_ADDITIONS\_H is
  set to 1 a user provided header file called freertos\_task\_c\_additions.h
  will be included at the bottom of tasks.c.  Functions defined in that
  header file can call freertos\_tasks\_c\_additions\_init(), which in turn
  calls a macro called FREERTOS\_TASKS\_C\_ADDITIONS\_INIT(), if it is defined.
  FREERTOS\_TASKS\_C\_ADDITIONS\_INIT() can be defined in FreeRTOSConfig.h.

+ Introduced configPRE\_SUPPRESS\_TICKS\_AND\_SLEEP\_PROCESSING( x ) which can be
...
+ Corrected the definition of StaticTask\_t in the case where
  INCLUDE\_xTaskAbortDelay is set to 1.

+ Introduced configTIMER\_SERVICE\_TASK\_NAME and configIDLE\_TASK\_NAME, both of
...
+ Allow the default human readable text names given to the Idle and Timer
  tasks to be overridden by defining the configIDLE\_TASK\_NAME and
  configTIMER\_SERVICE\_TASK\_NAME definitions respectively in FreeRTOSConfig.h.

+ Introduced configINITIAL\_TICK\_COUNT to allow the tick count to take a
  value of than 0 when the system boots.  This can be useful for
  testing purposes - although setting configUSE\_16\_BIT\_TICKS to 1 can also
...
+ Corrected use of portNVIC\_PENDSVSET to portNVIC\_PENDSVSET\_BIT in MPU ports.

+ Introduced configSTACK\_DEPTH\_TYPE to allow users to change the type used
  to specify the stack size when using xTaskCreate().  For historic reasons,
  when FreeRTOS was only used on small MCUs, the type was set to uint16\_t,
  but that can be too restrictive when FreeRTOS is used on larger
  processors.  configSTACK\_DEPTH\_TYPE defaults to uint16\_t.
  xTaskCreateStatic(), being a newer function, used a uint32\_t.
...
+ The prototype of the new xTaskCreateStatic() API function was modified to
  ...
  xTaskCreateStatic() is now uint32\_t, which changes the prototype of the
  callback functions.  See the following URL:
  https://www.FreeRTOS.org/xTaskCreateStatic.html

+ GCC ARM Cortex-A port:  Introduced the configUSE\_TASK\_FPU\_SUPPORT
  constant.  When configUSE\_TASK\_FPU\_SUPPORT is set to 2 every task is
...
+ ARM Cortex-M3/4F MPU ports:  Update to fully support the FreeRTOS V9.0.0
  ...
  FreeRTOS/Demo/CORTEX\_MPU\_Simulator\_Keil\_GCC demo application to
...
+ IAR32 GCC port:  Correct vPortExitCritical() when
  configMAX\_API\_CALL\_INTERRUPT\_PRIORITY == portMAX\_PRIORITY.
...
+ Introduced the configSUPPORT\_DYNAMIC\_ALLOCATION configuration constant to
  allow a FreeRTOS application to be built without a heap even being
...
+ Heap\_1.c and Heap\_2.c now use the configAPPLICATION\_ALLOCATED\_HEAP
  settings, which previously was only used by heap\_4.c.
  configAPPLICATION\_ALLOCATED_HEAP allows the application writer to declare
  the array that will be used as the FreeRTOS heap, and in-so-doing, place
  the heap at a specific memory location.

+ TaskStatus\_t structures are used to obtain details of a task.
  TaskStatus\_t now includes the bae address of the task's stack.

+ Added the vTaskGetTaskInfo() API function, which returns a TaskStatus\_t
...
+ Introduced configUSE\_TASK\_FPU\_SUPPORT, although the PIC32MZ EF port is
...
+ Minor updates to heap\_5.c to remove compiler warnings generated by some
  compilers.
...
+ PRIVILEGED\_FUNCTION and PRIVILEGED\_DATA macros, which are used in memory
...
+ Update heap\_4 and heap\_5 to allow use on 64-bit processors.
...
+ Correct calculation of xHeapStructSize in heap\_4 and heap\_5.
...
+ Introduce configAPPLICATION\_PROVIDES\_cOutputBuffer to allow FreeRTOS+CLI
...
+ Exclude the whole of croutine.c if configUSE\_CO\_ROUTINES is set to 0.

+ Change some data types from uint32\_t to size\_t in preparation for 64-bit
  Windows port.
...
+ NEW HEADER FILE REQUIRED!  Obsolete definitions have been separated into
  a new header file called FreeRTOS/Source/include/deprecated\_definitions.h.
...
+ Introduced configAPPLICATION\_ALLOCATED\_HEAP in heap\_4.c to allow the
  application writer to provide their own heap array - and in so doing
  control the location of the heap.

+ Introduced configUSE\_LIST\_DATA\_INTEGRITY\_CHECK\_BYTES which, when set, will
...
+ configASSERT()s in all Cortex-M ports used to test the lowest 5 bits of
  the interrupt control register to detect taskENTER\_CRITICAL() being called
  from an interrupt.  This has been changed to test all 8 bits.

+ Introduced uxTaskPriorityGetFromISR().

+ Microblze V8 port now tests XPAR\_MICROBLAZE\_0\_USE\_FPU for inequality to 0
...
Move the defaulting of configUSE\_PORT\_OPTIMISED\_TASK\_SELECTION into the
...
+ Introduce heap\_5.c, which is similar to heap\_4.c but allows the heap to
...
+ configUSE\_PORT\_OPTIMISED\_TASK\_SELECTION now defaults to 1 instead of 0.

+ Update all Cortex-M3/4F ports to trap a non interrupt safe API function
  being called from an interrupt handler.

+ Simplify the alignment checks in heap\_4.c.

+ Update the MSVC Windows simulator demo to use heap\_5.c in place of
  heap\_4.c to ensure end users have an example to refer to.
...
+ Introduce the pdMS\_TO\_TICKS() macro as a more user friendly and intuitive
  alternative to pdTICKS\_PER\_MS - both of which can be used to convert a
...
+ Call \_reclaim\_reent() when a task is deleted if configUSE\_NEWLIB\_REENTRANT
  is defined.


### Changes between V7.6.0 and V8.0.0 released 19th Feb 2014

https://www.FreeRTOS.org/Documentation/04-Roadmap-and-release-note/02-Release-notes/01-FreeRTOS-V8
...
See https://www.FreeRTOS.org/Documentation/04-Roadmap-and-release-note/02-Release-notes/01-FreeRTOS-V8 for full
information.
...
+ Add xPortGetMinimumEverFreeHeapSize() to heap\_4.c.
...
configUSE\_PREEMPTION is set to 0).  It is important to note that the
behaviour of the pre-emptive scheduler is unchanged - the following
description only applies when configUSE\_PREEMPTION is set to 0:

WHEN configUSE\_PREEMPTION IS SET TO 0 (which is in a small minority of
...
Blocked state or task A calls taskYIELD().  [If configUSE\_PREEMPTION is not
...
+ The Cortex-M0 version of portSET\_INTERRUPT\_MASK\_FROM\_ISR and
  portCLEAR\_INTERRUPT\_MASK\_FROM\_ISR are now fully nestable.

+ Improved behaviour and robustness of the default Cortex-M tickless idle
  behaviour.

+ Add workaround for silicon errata PMU\_CM001 in Infineon XMC4000 devices to
...
+ Ensure \_impure\_ptr is initialised before the scheduler is started.
...
+ Correct the \_\_NVIC\_PRIO\_BITS setting in the LPC18xx.h header files
...
function.  configUSE\_STATS\_FORMATTING\_FUNCTIONS must be set to 1 in
...
+ (MPU port only) The configINCLUDE\_APPLICATION\_DEFINED\_PRIVILEGED\_FUNCTIONS
...
+ Improve RAM usage efficiency of heap\_4.c and heap\_2.c.

+ Prevent heap\_4.c from attempting to free memory blocks that were not
  allocated by heap\_4.c, or have already been freed.
...
+ Added FreeRTOS+FAT SL source code and demo project.  The demo project
  ...
  experimentation and evaluation.  See https://www.FreeRTOS.org/fat\_sl
...
+ Updated the Cortex\_M4F\_Infineon\_XMC4500\_IAR demo to build with the latest
  version of EWARM.
...
+ Enhanced tickless support in the core kernel code with the introduction of
  the configEXPECTED\_IDLE\_TIME\_BEFORE\_SLEEP macro and the
...
+ Updated all heap\_x.c implementations to replace the structure that was
...
+ Added the portSUPPRESS\_TICKS\_AND\_SLEEP() macro and vTaskStepTick() helper
  function.

+ Added the configSYSTICK\_CLOCK\_HZ configuration constant.
...
+ Added a new example heap allocation implementation (heap\_4.c) that
...
+ Add #error macros into the Keil and IAR Cortex-M ports to ensure they
  cannot be built if the user has set configMAX\_SYSCALL\_INTERRUPT\_PRIORITY
  to 0.

+ Added comments in the FreeRTOSConfig.h files associated with Cortex-M3 and
  Cortex-M4 demos stating that the configMAX\_SYSCALL\_INTERRUPT\_PRIORITY
  parameter must not be set to 0.

+ Introduce new INCLUDE\_xQueueGetMutexHolder configuration constant
  (defaulted to 0).
...
+ Added a configASSERT() into the vPortFree() function in heap\_1.c as it is
...
+ Introduced the portSETUP\_TCB() macro to remove the requirement for the
  Windows simulator to use the traceTASK\_CREATE() macro, leaving the trace
  macro available for use by FreeRTOS+Trace (https://www.FreeRTOS.org/trace).

+ Added a new trace macro, traceMOVE\_TASK\_TO\_READY\_STATE(), to allow future
...
+ Changed the implementation of thread deletes in the Win32 port to prevent
  the port making use of the traceTASK\_DELETE() trace macros - leaving this
...
+ flash\_timer.c/h has been added to the list of standard demo tasks.  This
...
+ The portALIGNMENT\_ASSERT\_pxCurrentTCB() definition has been added to allow
...
+ The portCLEAN\_UP\_TCB() macro has been added to allow port specific clean
...
+ The legacy trace mechanism has been completely removed - it has been
  ...
  configuration constant configUSE\_TRACE\_FACILITY is now used to optionally
  ...
  output to provide more information.  When configUSE\_TRACE\_FACILITY is set
  to 1:
...
+ Added two new trace macros - traceTASK\_PRIORITY\_INHERIT() and
  traskTASK\_PRIORITY\_DISINHERIT().

+ Updated the traceQUEUE\_CREATE\_FAILED() macro to take a parameter that
  indicates the type of queue, mutex, or semaphore that failed to be
  created.

+ The position from which traceCREATE\_MUTEX() is called has been moved from
...
+ The value passed into tracePRIORITY\_SET() has been corrected for the case
...
+ portPOINTER\_SIZE\_TYPE has been introduced to prevent compiler warnings
...
+ A couple of new standard demos have been included.  First, a version of
  flop.c called sp\_flop.c.  This is similar to flop.c, but uses single
  ...
  is used.  Second, comtest\_strings.c has been included to allow the test
...
+ Updated the PIC32 port layer to ensure the
  configMAX\_SYSCALL\_INTERRUPT\_PRIORITY constant works as expected no matter
...
+ Added the traceTAKE\_MUTEX\_RECURSIVE\_FAILED() trace macro.

+ Removed the (pointless) parameter from the traceTASK\_CREATE\_FAILED()
  trace macro.

+ Introduced the portALT\_GET\_RUN\_TIME\_COUNTER\_VALUE() macro to compliment
  the already existing portGET\_RUN\_TIME\_COUNTER\_VALUE().  This allows for
...
+ Under specific conditions xFreeBytesRemaining within heap\_2.c could end up
...
+ All the contributed files that were located in the Demo/Unsupported\_Demos
...
+ The project file located in the Demo/CORTEX\_STM32F107\_GCC\_Rowley directory
...
+ portRESET\_PRIVILEGE() assembly macro updated to include a clobber list.
...
+ Corrected the name of portSVC\_prvRaisePrivilege to
  portSVC\_RAISE\_PRIVILEGE.

+ Added conditional compilation into xTaskGenericCreate() to prevent some
  compilers issuing warnings when portPRIVILEGE\_BIT is defined as zero.
...
+ New API function xPortGetFreeHeapSize() added to heap\_1.c and heap\_2.c.

+ ARM7 GCC demo interrupt service routines wrappers have been modified to
  call the C portion using an \_\_asm statement.  This prevents the function
...
+ ARM7 ports now automatically set the THUMB bit if necessary when
  ...
  THUMB\_INTERWORK to be defined.  This also allows THUMB mode and ARM mode
  tasks to be mixed more easily.

+ All ARM7/9 ports now have portBYTE\_ALIGNMENT set to 8 by default.
...
+ The linker scripts used with command line GCC demos have been updated to
  include an eh\_frame section to allow their use with the latest Yagarto
  release.  Likewise the demo makefiles have been updated to include
  command line options to reduce or eliminate the eh\_frame section all
  together.

+ The definition of portBYTE\_ALIGNMENT\_MASK has been moved out of the
...
+ Additional casting added to xTaskCheckForTimeOut().  This prevents
  problems that can arise should configUSE\_16\_BIT\_TICKS be set to 1 on a
  32 bit architecture (which would probably be a mistake, anyway).

+ Corrected the parameter passed to NVIC\_SetPriority() to set the MAC
  interrupt priority in both LPC1768 demos.

+ Decreased the default setting of configMINIMAL\_STACK\_SIZE in the PIC32
...
+ Set configUSE\_16\_BIT\_TICKS to 0 in the PPC405 demo projects.
...
+ Changed the Cortex M3 port.c asm statements to \_\_asm so it can be
...
+ Heap\_1.c and Heap\_2.c used to define structures for the purpose of data
...
+ BUG FIX - set the PIC32 definition of portSTACK\_GROWTH to the correct
  value of -1.
...
+ Added configMAX\_SYSCALL\_INTERRUPT\_PRIORITY usage to all the Cortex M3
...
+ Added the IntQTimer demo/test tasks to a demo project for each CM3 port
  (Keil, GCC and IAR) to test the new configMAX\_SYSCALL\_INTERRUPT\_PRIORITY
  functionality.

+ Added the mainINCLUDE\_WEB\_SERVER definition to the LM3SXXXX IAR and Keil
...
+ Added a new PowerPC port that demonstrates how the trace macros can be
  ...
  traceTASK\_SWITCHED\_OUT() and traceTASK\_SWITCHED\_INT() macros are used to
...
+ Added the queue registry feature.  The queue registry is provided as a
  ...
  will only be used when configQUEUE\_REGISTRY\_SIZE is greater than zero.
...
+ Implemented the configKERNEL\_INTERRUPT\_PRIORITY within the Keil Cortex
...
+ Changed the method used to force a context switch within an ISR for the
  ARM7/9 GCC ports only.  The portENTER\_SWITCHING\_ISR() and
  portEXIT\_SWITCHING\_ISR() macros are no longer supported.  This is to
...
+ Updated the GCC port for the Cortex M3 to include the
  configKERNEL\_INTERRUPT\_PRIORITY functionality.  This was previously only
...
+ Updated listGET\_OWNER\_OF\_NEXT\_ENTRY() to assist compiler optimisation
...
+ Introduced configKERNEL\_INTERRUPT\_PRIORITY to the IAR Cortex-M3, PIC24
...
+ Moved location of call to LCD\_Init() in STR9 demo to ensure it is only
...
+ Tasks that block on events with a timeout of portMAX\_DELAY are now
  blocked indefinitely if configINCLUDE\_vTaskSuspend is defined.
  Previously portMAX\_DELAY was just the longest block time possible. This
  is still the case if configINCLUDE\_vTaskSuspend is not defined.
...
+ Corrected the value passed to the WDG\_PeriodValueConfig() library
...
+ The EMAC driver in the SAM7X lwIP demo has been made more robust as per
  the thread: https://sourceforge.net/forum/message.php?msg\_id=3714405
...
+ In the Borland x86 ports there was a mistake in the portFIRST\_CONTEXT
...
+ Changed the WinAVR makefile to make chars signed - needed for the
  co-routine code if BaseType\_t is set to char.
...
+ The various LPC2000 ports had a mistake in the timer setup where the
  prescale value was written to T0\_PC instead of T0\_PR.  This would have
...
+ Updated the HCS12 port to support banking and introduced a demo
  ...
  located in the Demo/HCS12\_CodeWarrior\_banked directory.

+ The name of the directory containing the MC9S12F32 demo application
  has been changed to Demo/HCS12\_CodeWarrior\_small (as in 'small'
  memory model).
...
+ Added a new conditional compilation macro configIDLE\_SHOULD\_YIELD.
...
+ Kernel and miscellaneous changes


#### API changes

+ Each port now defines BaseType\_t as the data type that is most
  efficient for that architecture.  The type BaseType\_t is used
...
+ The source/portable/GCC/ARM7 directory has been renamed
  source/portable/GCC/ARM7\_LPC2000 so it is compatible with the naming
  of other GCC ARM7 ports.

+ The Demo/PIC directory has been renamed Demo/PIC18\_MPLAB to
  accommodate the wizC/fedC PIC port.

+ The demo applications for the two AVR ports no longer share the same
  directory.  The WinAVR demo is in the Demo/AVR\_ATMega323\_WinAVR
  directory and the IAR port in the Demo/AVR\_ATMega323\_IAR directory.
...
+ Introduced configUSE\_IDLE\_HOOK in idle task.
...
+ Corrected spelling of portMINMAL\_STACK\_SIZE to portMINIMAL\_STACK\_SIZE.
...
+ vTaskStartScheduler() no longer takes the configUSE\_PREEMPTION
...
+ pvPortMalloc() now takes a size\_t parameter as per the ANSI malloc().
...
+ Introduced the portTASK\_FUNCTION() and portTASK\_FUNCTION\_PROTO()
  macros.
...
+ tskMAX\_TASK\_NAME\_LEN removed from the task.h header and added to each
  individual portmacro.h file as portMAX\_TASK\_NAME\_LEN.  This allows RAM
...
+ GCC LPC2106 port - Corrected the optimisation options in the batch files
  ROM\_THUMB.bat, RAM\_THUMB.bat, ROM\_ARM.bat and RAM\_ARM.bat.  The lower case
...
+ SAM7 USB driver - Replaced the duplicated RX\_DATA\_BK0 in the interrupt
  mask with the RX\_DATA\_BK1.
...
+ Added INCLUDE\_vTaskDelay conditional compilation.

+ Changed the name of the Demo/ARM7\_AtmelSAM7S64\_IAR directory to
  Demo/ARM7\_AT91SAM7S64\_IAR for consistency.
...
+ Renamed the Demo/ARM7 directory to Demo/ARM7\_LPC2106\_GCC.

+ Renamed the Demo/ARM7\_Keil directory to Demo/ARM7\_LPC2129\_Keil.
...
+ The memory pool allocated within heap\_1.c has been placed within a
...
+ Changed the name of the constant portTICKS\_PER\_MS to portTICK\_PERIOD\_MS
...
+ Added heap\_1.c, heap\_2.c and heap\_3.c in the Source/Portable/MemMang
  directory.
...
+ Updated the portmacro.h file for each demo application to include the
  constants required for the new memory allocators: portTOTAL\_HEAP\_SIZE and
  portBYTE\_ALIGNMENT.

+ Added a new test to the ARM7 demo application that tests the operation
  of the heap\_2 memory allocator.
...
+ Rationalised the ARM7 port version of portEXIT\_CRITICAL() -
...
+ The typedef TickType\_t has been introduced.  All delay times should
  now use a variable of type TickType\_t in place of the unsigned long's
  used previously.  API function prototypes have been updated
  appropriately.

+ The configuration macro USE\_16\_BIT\_TICKS has been introduced.  If set
  to 1 TickType\_t is defined as an unsigned short.  If set to 0
  TickType\_t is defined as an unsigned long.  See the configuration
  section of the API documentation for more details.

+ The configuration macro INCLUDE\_vTaskSuspendAll is now obsolete.
...
+ ulTaskGetTickCount() has been renamed xTaskGetTickCount() as the type
  it returns now depends on the USE\_16\_BIT\_TICKS definition.
...
+ Introduced portGLOBAL\_INTERRUPT\_FLAG definition to test the global
  interrupt flag setting.  Using the two bits defined within
  portINITAL\_INTERRUPT\_STATE was causing the w register to get clobbered
...
+ The ATMega port definition of portCPU\_CLOCK\_HZ definition changed to
...
+ sTaskCreate() has been modified slightly to make use of the
  portSTACK\_GROWTH macro.  This is required for the PIC port where the
...
+ Corrected the macro naming of portMS\_PER\_TICK to portTICKS\_PER\_MS.
...
+ Renamed portTICKS\_PER\_MS definition to portMS\_PER\_TICK (milli seconds
...
+ The definition tskMAX\_PRIORITIES has been moved from task.h to
  portmacro.h and renamed portMAX\_PRIORITIES.  This allows different
  ports to allocate a different maximum number of priorities.

+ By default the trace facility is off, previously USE\_TRACE\_FACILITY
  was defined.
...
+ portSWITCH\_CONTEXT() replaced by vPortFirstContext().
